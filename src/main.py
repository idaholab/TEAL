...
try:
  ...
  # In general, this should not exist, and RAVEN should import CashFlow.CashFlowExtMod instead of importing CashFlowExtMod directly, implicitly.
except (ImportError, ModuleNotFoundError):
  import CashFlows

ravenPath= os.path.abspath(os.path.dirname(__file__)) + '/../../raven/framework'
sys.path.append(ravenPath) #'~/projects/raven/framework') # TODO generic RAVEN location
...
def checkRunSettings(settings, components):
  ...
  for find, findCf in settings.getActiveComponents().items():
...
def checkDrivers(settings, components, variables, v=100):
  ...
  """
  m = 'check_drivers'
  #active = _getActiveDrivers(settings, components)
  ...
  for comp in components:
    ...
    for c, cf in enumerate(comp.getCashflows()):
      ...
      if driver is None or utils.isAFloatOrInt(driver) or isinstance(driver, np.ndarray):
        ...
        if found:
          if driverCf not in list(mCf.name for mCf in matchComp.getCashflows()):
            found = False
      if not found:
...
def projectLifeCashflows(settings, components, lifetimeCashflows, projectLength, v=100):
  """
...
def projectSingleCashflow(cf, start, end, life, lifeCf, taxMult, inflRate, projectLength, v=100):
  ...
  operatingMask = np.logicalAnd(years >= start, years <= end)
  operatingYears = years[operatingMask]
  startShift = operatingYears - start # yShift
  ...
  if operatingYears[-1] < years[-1]:
    decomissionMask[0] = np.hstack((decomissionMask[0],np.atleast1d(operatingYears[-1]+1)))
...
def FCFF(components, cashFlows, projectLength, mult=None, v=100):
  """
...
def NPV(components, cashFlows, projectLength, discountRate, mult=None, v=100, returnFcff=False):
  """
