...
class Component:
  ...
  def getInputSpecs(cls):
    ...
    comp.addSub(InputData.parameterInputFactory('Start_time', contentType=InputTypes.IntegerType))
  ...
  def readInput(self, source):
    ...
    cfs = specs.findFirst('Cash_flows')
    if cfs is not None:
      for sub in cfs.subparts:
        newCfs = self._cashFlowFactory(sub) #Cash_flow(self.name, verbosity=self._verbosity)
  ...
  def getStartTime(self):
    """
      Get the startTime for this component
      @ In, None
      @ Out, startTime, int, the start time of this component
    """
...
class Recurring(CashFlow):
  ...
  def calculateCashflow(self, variables, lifetimeCashflows, lifetime, verbosity):
    ...
    #assert self.YearlyCashflow is not None
